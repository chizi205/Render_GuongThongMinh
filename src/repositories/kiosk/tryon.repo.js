...
const markTempUsed = async ({ temp_image_id }) => {
  await db.query(`UPDATE tryon_temp_images SET is_used = true WHERE id = $1`, [
    temp_image_id,
  ]);
};

async function updateCustomerImageUrl(sessionId, imageUrl) {
  let relativeUrl = imageUrl;
  const uploadIndex = imageUrl.indexOf("/uploads");
  if (uploadIndex !== -1) {
    relativeUrl = imageUrl.substring(uploadIndex);
  }
  const query = `
    UPDATE shop_users 
    SET image_url = $1 
    WHERE user_id = (SELECT user_id FROM kiosk_sessions WHERE id = $2)
      AND shop_id = (SELECT shop_id FROM kiosk_sessions WHERE id = $2)
    RETURNING user_id;
  `;
  const res = await db.query(query, [relativeUrl, sessionId]);
  return res.rows[0];
}
async function getUserBySession(sessionId) {
  const query = `
    SELECT ks.shop_user_id, su.user_id, ks.shop_id 
    FROM kiosk_sessions ks
    JOIN shop_users su ON ks.shop_user_id = su.id
    WHERE ks.id = $1
  `;
